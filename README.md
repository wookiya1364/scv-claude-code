...
| Command | What | When |
|---|---|---|
| **`/scv:help`** | Current status + recommended next action | **When unsure what to do — always first** |
| **`/scv:status`** | Raw changes + active promote plans + epic progress | After dropping files into `scv/raw/` |
| **`/scv:promote`** | Refines `scv/raw/` into a `scv/promote/<slug>/` folder. Auto-suggests splitting big requests into smaller features grouped under one `epic` (count chosen per content, not fixed) | When raw materials accumulate |
| **`/scv:work`** | Implements a promote plan: reads PLAN + TESTS, runs tests, archives on pass, can also create the PR (with screenshots) | When starting implementation of a promote plan |
| **`/scv:regression`** | Runs accumulated regression across `scv/archive/**/TESTS.md`. Auto-skips slugs declared via `supersedes:` or marked `obsolete`. Triages failures via dialog | Periodically, before releases, or right before archiving |
| **`/scv:report`** | Posts a phase report to Slack/Discord | When sharing results with the team |
| **`/scv:sync`** | Applies plugin updates to your project | After plugin version bumps |

Just type the command — Claude handles the rest by asking when needed. No flags to memorize.

> **Often-confused pair**
> - **`/scv:status`** = "What's the current state?" (read-only — safe anytime, never modifies files)
> - **`/scv:promote`** = "Let me organize this raw material" (interactive — creates PLAN + TESTS scaffolds)
>
> When unsure, always start with `/scv:help`.
...
| 커맨드 | 역할 | 언제 쓰나요 |
|---|---|---|
| **`/scv:help`** | 현재 상태 진단 + 다음 액션 추천 | **뭘 해야 할지 모를 때, 항상 가장 먼저** |
| **`/scv:status`** | `scv/raw/` 변경 + 활성 promote 계획 + epic 진척도 | raw 에 파일 넣은 뒤 확인할 때 |
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<slug>/` 정제. 거대 요구는 적절한 갯수의 feature 로 쪼개 같은 epic 으로 묶을지 자동 제안 (분할 갯수는 raw 내용에 맞춰 결정) | raw 에 새 자료가 쌓였을 때 |
| **`/scv:work`** | promote 계획 구현 — PLAN + TESTS 읽고 구현 → 테스트 → 통과 시 archive + (스크린샷 첨부한) PR 생성도 가능 | 구현 시작할 때 |
| **`/scv:regression`** | `scv/archive/**/TESTS.md` 누적 회귀. `supersedes:` 선언되거나 `obsolete` 마킹된 slug 는 자동 skip. 실패 시 대화로 triage | 주기적, 릴리즈 전, 또는 archive 직전 |
...
> **자주 헷갈리는 둘**
> - **`/scv:status`** = "지금 뭐가 있나?" (읽기만 — 언제든 안전, 파일 안 건드림)
> - **`/scv:promote`** = "이 raw 자료를 정리하자" (대화로 PLAN + TESTS scaffold 생성)
>
> 모르겠으면 항상 `/scv:help` 부터.
...
| コマンド | 役割 | いつ使う |
|---|---|---|
| **`/scv:help`** | 状態診断 + 次のアクション推奨 | **何をすべきか分からないとき — 必ず最初** |
| **`/scv:status`** | `scv/raw/` の変更 + アクティブな promote + epic 進捗 | raw にファイルを投入した後 |
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<slug>/` へ精製。大きな要求は適切な数の feature に分割し同じ epic でまとめるかを自動提案 (分割数は raw の内容に応じて決定) | raw に資料が溜まったとき |
| **`/scv:work`** | promote 計画を実装 — PLAN + TESTS を読んで実装 → テスト → archive + (スクリーンショット付き) PR 作成も可能 | 実装開始時 |
| **`/scv:regression`** | `scv/archive/**/TESTS.md` の累積回帰。`supersedes:` または `obsolete` マークされた slug は自動 skip。失敗時は対話で triage | 定期的・リリース前・archive 直前 |
...
> **よく混同される 2 つ**
> - **`/scv:status`** = "今の状態は?" (読み取り専用 — いつでも安全、ファイルは触らない)
> - **`/scv:promote`** = "この raw 資料を整理する" (対話で PLAN + TESTS scaffold 生成)
>
> 迷ったらまず `/scv:help`。
