...
| **`/scv:promote`** | Refines `scv/raw/` into a `scv/promote/<slug>/` folder. Auto-suggests splitting big requests into 5~7 features grouped under one `epic` | When raw materials accumulate |
...
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<slug>/` 정제. 거대 요구는 5~7 개 feature 로 쪼개고 같은 epic 으로 묶을지 자동 제안 | raw 에 새 자료가 쌓였을 때 |
...
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<slug>/` へ精製。大きな要求は 5~7 個の feature に分割し同じ epic でまとめるかを自動提案 | raw に資料が溜まったとき |
