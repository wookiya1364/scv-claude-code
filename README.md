...
| **`/scv:status`** | Raw changes + active promote plans + docs graph state | After dropping files into `scv/raw/` |
| **`/scv:promote`** | Refines `scv/raw/` items into `scv/promote/<YYYYMMDD>-<author>-<slug>/` with `PLAN.md` + `TESTS.md`; also refreshes the docs graph | When raw materials accumulate |
| **`/scv:work`** | Implements a promote plan: reads PLAN + TESTS, runs tests, offers to archive on pass | When starting implementation of a promote plan |
| **`/scv:regression`** | Runs accumulated regression across `scv/archive/**/TESTS.md` (+ optional promote). Auto-skips slugs declared via `supersedes:` or marked `obsolete`. Triages failures via 3-way dialog (regression / obsolete / flaky) | Periodically, before releases, or right before archiving a new plan |
| **`/scv:report`** | Posts phase report to Slack/Discord | When sharing results with the team |
| **`/scv:sync`** | Applies plugin updates to your project | After plugin version bumps |

All commands support `--help` / `-h`. Example: `/scv:report --help`.
...
| **`/scv:status`** | `scv/raw/` 변경 + 활성 promote 계획 + 그래프 상태 | raw 에 파일 넣은 뒤 확인할 때 |
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<YYYYMMDD>-<author>-<slug>/` 로 정제 (PLAN.md + TESTS.md 생성) + 그래프 갱신 | raw 에 새 자료가 쌓였을 때 |
| **`/scv:work`** | promote 계획 구현 — PLAN + TESTS 읽고 구현 → 테스트 실행 → 통과 시 archive 대화 | 구현 시작할 때 |
| **`/scv:regression`** | `scv/archive/**/TESTS.md` 누적 회귀 실행. `supersedes:` 선언되거나 `obsolete` 마킹된 slug 는 자동 skip. 실패 시 3-way triage (regression / obsolete / flaky) | 주기적으로, 릴리즈 전에, 또는 새 계획 archive 직전에 |
| **`/scv:report`** | Slack/Discord 에 Phase 결과 보고 | 구현·테스트 결과를 팀에 공유할 때 |
| **`/scv:sync`** | 플러그인 업데이트를 내 프로젝트에 반영 | 플러그인 버전이 올라간 뒤 |

각 커맨드에 `--help` / `-h` 옵션이 있습니다. 예: `/scv:report --help`.
...
| **`/scv:status`** | `scv/raw/` の変更 + アクティブな promote 計画 + グラフ状態 | raw にファイルを投入した後 |
| **`/scv:promote`** | `scv/raw/` → `scv/promote/<YYYYMMDD>-<author>-<slug>/` へ精製 (PLAN.md + TESTS.md 生成) + グラフ更新 | raw に資料が溜まったとき |
| **`/scv:work`** | promote 計画を実装 — PLAN + TESTS を読んで実装 → テスト実行 → 通過時 archive 対話 | 実装開始時 |
| **`/scv:regression`** | `scv/archive/**/TESTS.md` の累積回帰を実行。`supersedes:` 宣言または `obsolete` マークされた slug は自動 skip。失敗時は 3-way triage (regression / obsolete / flaky) | 定期的・リリース前・新規計画の archive 直前 |
| **`/scv:report`** | Slack / Discord に Phase 結果レポート | 実装・テスト結果をチームに共有するとき |
| **`/scv:sync`** | プラグイン更新を自分のプロジェクトに反映 | プラグインバージョンアップ後 |

すべて `--help` / `-h` に対応。例: `/scv:report --help`。
