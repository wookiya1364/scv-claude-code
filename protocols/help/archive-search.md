...
1. Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/help.sh" --archive-index` and read its `ARCHIVE_INDEX:` block (`<folder> | <title> | <created_at>` per entry). If it shows `(empty)` or `(no archive yet)`, tell the user honestly and suggest `/scv:promote` to create the first plan. Stop.
