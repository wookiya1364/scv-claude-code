...
mode()  { WS_CLAUDE="$1/scv/CLAUDE.md" WS_MANIFEST="$1/scv/WORKSPACE.yaml" bash -c 'source "'"$LIB"'"; scv_resolve_mode'; }
field() { WS_CLAUDE="$1/scv/CLAUDE.md" bash -c 'source "'"$LIB"'"; '"$2"; }
...
[[ -f "$P1/scv/CLAUDE.md" ]] && ok "scv/CLAUDE.md created" || fail "scv/CLAUDE.md missing"
...
replace_marker_block "$P1/scv/CLAUDE.md" "PROJECT:LOCAL START" "PROJECT:LOCAL END" "MY-LOCAL-RULE-XYZ"
...
grep -q "MY-LOCAL-RULE-XYZ" "$P1/scv/CLAUDE.md" && ok "PROJECT:LOCAL survives sync" || fail "PROJECT:LOCAL lost on sync"

# 5. DETACH — clear root → SINGLE again, no migration
replace_marker_block "$P1/scv/CLAUDE.md" "SCV:WORKSPACE START" "SCV:WORKSPACE END" \
  '```yaml
repo_id: be
role: backend
root:
