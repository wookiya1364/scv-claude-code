...
Wrapper와 Core는 서로 독립적으로 릴리스합니다. 현재 Claude 어댑터는
`0.20.2`이고 Core `0.20.3`을 pin합니다. Core sync는 체크섬 pin과 생성
projection만 갱신하며 wrapper `VERSION`, plugin manifest, marketplace
version은 변경하지 않습니다.
